...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(
      '.tmp-popup-mobile-menu .close-button, .tmp-popup-mobile-menu a[href^="#"]',
    ).on("click", function () {
      closeMobileMenu();
    });
    ...
    const modalDialog = modal
      ? modal.querySelector(".education-modal__dialog")
      : null;

    function openEducationModal(item) {
      if (!modal) return;
      modalTitle.textContent =
        item.getAttribute("data-detail-title") || "Подробности";
      modalText.textContent =
        item.getAttribute("data-detail-text") ||
    ...
    document
      .querySelectorAll("#education .resume-single")
